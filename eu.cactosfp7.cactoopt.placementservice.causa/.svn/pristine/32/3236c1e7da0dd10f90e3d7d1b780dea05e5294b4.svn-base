...
import se.umu.cs.ds.causa.constraints.local.LocalConstraint;
...
//  public static final String PREFIX_ULM_COMPUTENODENAMES = "PM";
  ...
  private static PhysicalMachine.Label[] getPMArchitectureLabels (String nodename)
  {
    ...
    switch (index)
    {
      ...
      case  8 : return new PhysicalMachine.Label[] {PMLABEL_ARCHITECTURE_SANDYBRIDGE};
  ...
  {
    final int RESERVATION_STORAGE = 1;
...
//    list.add(StorageReservationGlobalConstraint.getInstance(1));
    for (LocalConstraint lc : NoCPUCoreOverCommitGlobalConstraint.SINGLETON.deriveLocalConstraints(datacenter.getPhysicalMachines()))
      list.add(lc);
    for (LocalConstraint lc : NoRAMOverCommitGlobalConstraint.SINGLETON.deriveLocalConstraints(datacenter.getPhysicalMachines()))
      list.add(lc);
    for (LocalConstraint lc : StorageReservationGlobalConstraint.getInstance(RESERVATION_STORAGE).deriveLocalConstraints(datacenter.getPhysicalMachines()))
      list.add(lc);
    ...
    for (VirtualMachine vm : datacenter.getVirtualMachines())
    {
      ...
      if (pms.length > 0)
      {
        ...
        // VM state constraints (immovable)
        if (vm.hasLabel(VMLABEL_IMMOVABLE))
          list.add(PMAffinityVirtualMachineConstraint.getInstance(vm.getId(),pm.getId()));
